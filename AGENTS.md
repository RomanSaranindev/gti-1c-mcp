...
| `get_user_rights` | Группы доступа и профили пользователя по **ссылке** (`user_ref`). Формат: `Справочник.Пользователи?ref=<hex>`. ФИО не передаётся. Параметры: `user_ref` (обязательный), `include_access_values` (`true`/`false`), `limit` |
