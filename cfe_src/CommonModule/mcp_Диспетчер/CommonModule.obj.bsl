...
Функция ОбработатьInitialize(Параметры)
	...
	ИнформацияСервера.Вставить("version", ВерсияСервера());
...
// Версия сервера — из свойств расширения; запасное значение на случай,
// если версия в свойствах не заполнена.
Функция ВерсияСервера()

	Расширение = Метаданные.ОбщиеМодули.mcp_Диспетчер.РасширениеКонфигурации();
	Если Расширение <> Неопределено И Не ПустаяСтрока(Расширение.Версия) Тогда
